...
export default function CreateBoardForm({ onSubmit, disabled }: CreateBoardFormProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Board sections</label>
            ...
            {sections.map((section, index) => (
              <div key={section.id} className="flex items-center gap-2">
                ...
                <input
                  ...
                  maxLength={50}
                />
                <ColorPicker value={section.color} onChange={color => updateSection(section.id, { color })} />
                <button
                  type="button"
        ...
        <Button
          type="submit"
          disabled={disabled || sections.filter(s => s.name.trim()).length === 0}
